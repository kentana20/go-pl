...
インタフェースは統一的に扱わなければならない2つ以上の具象型が存在する場合にのみ必要。外部に公開するメソッド、構造体を絞りたい場合は公開の仕組みを使うべき。

インタフェースは特定の実装から離れて抽象化を行うので、io.Writer, fmt.Stringerインタフェースのように1つのメソッドを持つケースが多い。

インタフェース設計に対する優れた経験則は、必要なものだけを求めること。
